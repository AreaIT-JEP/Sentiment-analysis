...
class SentimentAnalysisApp:
    def __init__(self, root):
        ...
        # Use more workers on machines with more cores
        self.max_workers = max(2, min(8, os.cpu_count() or 4))
        
        # Track chunked reading for large files
        self.df = None
        self.products = {}
    ...
    def setup_ui(self):
        ...
        # Advanced options
        adv_frame = ttk.LabelFrame(control_frame, text="Analysis Options")
        adv_frame.pack(side=tk.LEFT, padx=20, pady=5)
        
        # Add concurrency slider
        ttk.Label(adv_frame, text="Threads:").pack(side=tk.LEFT, padx=5)
        self.thread_var = tk.IntVar(value=self.max_workers)
        ...
        ttk.Label(adv_frame, textvariable=self.thread_var).pack(side=tk.LEFT, padx=2)

        # Theme toggle button
        self.btn_theme = ttk.Button(control_frame, text="Toggle Theme", command=self.toggle_theme)
        self.btn_theme.pack(side=tk.RIGHT, padx=5, pady=5)
    ...
    def apply_theme(self):
        ...
        # Update button text
        self.btn_theme.config(text="Dark Theme" if self.current_theme == "light" else "Light Theme")
    ...
    def _create_charts_batch(self, items):
        """Create charts in batches for better performance."""
        theme = THEMES[self.current_theme]
        ...
        # Take only top 50 products by review count
        items = items[:50]
        
        # Process in batches of 5 charts at a time
        batch_size = 5
        for batch_start in range(0, len(items), batch_size):
            batch = items[batch_start:batch_start + batch_size]
            
            for product, data in batch:
                # Create card frame for this product
